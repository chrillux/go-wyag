...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.go-wyag.yaml)")
}
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name ".go-wyag" (without extension).
		viper.AddConfigPath(home)
		viper.SetConfigType("yaml")
		viper.SetConfigName(".go-wyag")
	}
